...
class CiviCRM_Caldera_Forms_Membership_Processor {
	...
	public function register_processor( $processors ) {
		...
		$processors[$this->key_name] = [
			...
			'description' =>  __( 'Create CiviCRM Memberhips', 'caldera-forms-civicrm' ),
	...
	public function pre_processor( $config, $form, $processid ) {
		...
		if ( isset( $is_member ) && ! $member['is_error'] )
			return [
				'note' => __( 'It looks like you already have a membership.', 'caldera-forms-civicrm' ), 
				'type' => 'error'
			];
		...
		if ( ! empty( $transient->contacts->{$this->contact_link} ) ) {
			...
			if ( ! $config['is_renewal'] ) {
				$form_values['join_date'] = $form_values['join_date'] ? $form_values['join_date'] : date('Y-m-d');
				$form_values['start_date'] = $form_values['start_date'] ? $form_values['start_date'] : date('Y-m-d');
			}
			if ( $config['is_renewal'] && $is_member ) {
				$form_values = array_merge( $form_values, $is_member );
				unset( $form_values['is_error'] );
			}
